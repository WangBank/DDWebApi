//------------------------------------------------------------------------------
// <auto-generated>
//     此代码由工具 FreeSql.Generator 生成。
//     运行时版本:3.1.5
//     Website: https://github.com/2881099/FreeSql
//     对此文件的更改可能会导致不正确的行为，并且如果
//     重新生成代码，这些更改将会丢失。
// </auto-generated>
//------------------------------------------------------------------------------
using System;
using System.Collections;
using System.Collections.Generic;
using System.Linq;
using System.Reflection;
using System.Threading.Tasks;
using Newtonsoft.Json;
using FreeSql.DataAnnotations;
namespace DingtalkApprovalApi.Entities.Dtos {

	[JsonObject(MemberSerialization.OptIn)]
	public partial class DataItem {

		[JsonProperty, Column(DbType = "varchar(50)", IsPrimary = true)]
		public string Guid { get; set; } = string.Empty;

		[JsonProperty, Column(DbType = "varchar(50)")]
		public string billtype { get; set; } = string.Empty;

		[JsonProperty, Column(DbType = "varchar(50)")]
		public string BranchGuid { get; set; } = string.Empty;

		[JsonProperty, Column(DbType = "varchar(200)")]
		public string Char1 { get; set; } = string.Empty;

		[JsonProperty, Column(DbType = "varchar(100)")]
		public string Char2 { get; set; } = string.Empty;

		[JsonProperty, Column(DbType = "varchar(100)")]
		public string Char3 { get; set; } = string.Empty;

		[JsonProperty, Column(DbType = "varchar(100)")]
		public string Char4 { get; set; } = string.Empty;

		[JsonProperty, Column(DbType = "varchar(100)")]
		public string Char5 { get; set; } = string.Empty;

		[JsonProperty, Column(DbType = "nvarchar(100)")]
		public string ChtName { get; set; } = string.Empty;

		[JsonProperty, Column(DbType = "varchar(50)")]
		public string Code { get; set; } = string.Empty;

		[JsonProperty, Column(DbType = "varchar(50)")]
		public string DataItemTypeGuid { get; set; } = string.Empty;

		[JsonProperty, Column(DbType = "varchar(100)")]
		public string EnglishName { get; set; } = string.Empty;

		[JsonProperty, Column(DbType = "decimal(18,6)")]
		public decimal? Float1 { get; set; }

		[JsonProperty, Column(DbType = "decimal(18,6)")]
		public decimal? Float2 { get; set; }

		[JsonProperty, Column(DbType = "decimal(18,6)")]
		public decimal? Float3 { get; set; }

		[JsonProperty, Column(DbType = "decimal(18,6)")]
		public decimal? Float4 { get; set; }

		[JsonProperty, Column(DbType = "decimal(18,6)")]
		public decimal? Float5 { get; set; }

		[JsonProperty, Column(DbType = "nvarchar(500)")]
		public string FullName { get; set; } = string.Empty;

		[JsonProperty, Column(DbType = "varchar(100)")]
		public string HelpCode { get; set; } = string.Empty;

		[JsonProperty]
		public bool? iscostinvolved { get; set; }

		[JsonProperty]
		public bool? IsForbidden { get; set; }

		[JsonProperty]
		public short? IsLeaf { get; set; }

		[JsonProperty]
		public bool? IsSystem { get; set; }

		[JsonProperty, Column(DbType = "nvarchar(100)")]
		public string Name { get; set; } = string.Empty;

		[JsonProperty, Column(DbType = "nvarchar(100)")]
		public string Notes { get; set; } = string.Empty;

		[JsonProperty, Column(DbType = "varchar(50)")]
		public string ParentGuid { get; set; } = string.Empty;

		[JsonProperty, Column(DbType = "image")]
		public byte[] Picture { get; set; }

	}

}
